...
"""
Evaluates and manages risk parameters for trade strategies.
"""
...
class RiskEvaluator:
    def __init__(self, parameters: Dict[str, Any]) -> None:
        """
        parameters: {
          "risk_pct": float,                # percent of equity to risk per trade
          "min_reward_risk_ratio": float,   # minimum acceptable reward:risk
    ...
    def calculate_position_size(
            self, equity: float, stop_loss: float, risk_pct: float
    ) -> float:
        risk_amount = equity * risk_pct / 100.0
        size = risk_amount / abs(stop_loss)
        self.logger.info(
            "Position size calc: equity=%.2f, stop_loss=%.4f, risk_pct=%.2f → size=%.4f",
    ...
    def evaluate(self, strategy: Dict[str, Any]) -> bool:
        """
        Returns True if the strategy passes risk checks.
        Expects strategy to include 'entry', 'stop_loss', 'take_profit'.
        """
